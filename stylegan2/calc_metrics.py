...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, gen_folder=args.gen_folder)
        if rank == 0:
...
@click.option('--gen_folder', help='path to image gen folder if existed', type=str, default=None)
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose, gen_folder):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, gen_folder=gen_folder)
    ...
    # args.dataset_kwargs.resolution = 64
